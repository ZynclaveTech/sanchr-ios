...
final class MediaChainState: @unchecked Sendable {
    ...
    var deviceSecretData: Data { deviceSecret }

    init(deviceSecret: Data) {
    ...
    func getOrInitChainKey(conversationId: String) -> Data {
        lock.lock()
        defer { lock.unlock() }
    ...
    @discardableResult
    func advanceChainKey(conversationId: String) -> Data {
        lock.lock()
        defer { lock.unlock() }
