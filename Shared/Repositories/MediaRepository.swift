...
protocol MediaRepositoryProtocol: AnyObject, Sendable {
    ...
    func getUploadUrl(fileSize: Int64, contentType: String, sha256Hash: String) async throws -> Sanchr_Media_PresignedUrlResponse

    /// Gets a presigned URL for downloading an encrypted media blob.
    func getDownloadUrl(mediaId: String) async throws -> Sanchr_Media_PresignedUrlResponse

    /// Confirms that a media upload completed successfully.
    func confirmUpload(mediaId: String, fileSize: Int64) async throws -> Sanchr_Media_ConfirmUploadResponse
}
...
final class MediaRepositoryImpl: MediaRepositoryProtocol, @unchecked Sendable {
    ...
    func getUploadUrl(fileSize: Int64, contentType: String, sha256Hash: String) async throws -> Sanchr_Media_PresignedUrlResponse {
        ...
        var request = Sanchr_Media_GetUploadUrlRequest()
    ...
    func confirmUpload(mediaId: String, fileSize: Int64) async throws -> Sanchr_Media_ConfirmUploadResponse {
        ...
        var request = Sanchr_Media_ConfirmUploadRequest()
        request.mediaID = mediaId
        request.fileSize = fileSize
