...
protocol KeyRepositoryProtocol: AnyObject, Sendable {
    ...
    func uploadKeyBundle(_ bundle: Sanchr_Keys_KeyBundle) async throws

    /// Fetches the pre-key bundle for a specific user and device from the server.
    func getPreKeyBundle(userId: String, deviceId: Int32) async throws -> Sanchr_Keys_PreKeyBundleResponse

    /// Uploads additional one-time pre-keys to the server.
    func uploadOneTimePreKeys(_ keys: [Sanchr_Keys_OneTimePreKey]) async throws -> Int32
