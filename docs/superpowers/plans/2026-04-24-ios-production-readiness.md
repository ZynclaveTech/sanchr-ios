...
| 2026-04-25 | P0 Calls | Landed. 28 commits across 6 sub-phases (A cleanup → B proto evolution → C inbound decrypt → D outbound fan-out → E in-call signaling → F video flag). Full unit suite 374/0/2. Every hardcoded `deviceId: 1` / `senderDevice: 1` site in `CallManager.swift` is gone. iOS side complete; multi-device calls begin working end-to-end once backend ships the four counterpart changes (populate `caller_device`, consume `device_offers`, mirror `answerer_device` → `peer_device`, include `caller_device` in APNs payload). Until then iOS falls back to device 1 across the board, so behavior is a strict superset of the prior state. Three notable lessons that should propagate to subsequent phases: (1) `Scripts/generate-protos.sh` is unsafe in this repo — generated SwiftProtobuf files are hand-edited per established team practice, and regen would delete `DeleteConversation` symbols other code depends on; document this as a P5 hygiene task. (2) Tests that exercise `CallManager.startCall` skip silently in the iOS Simulator because `WebRTCClient.createOffer()` throws first — extract a pure static helper and test it directly (we did this for `buildOutgoingCallOffer`, `buildEncryptedAnswerPayload`, `decryptIncomingAnswer`, `shouldBlockVideoCall`). (3) `#if DEBUG` / `testOnly_` shims are the WRONG pattern for this codebase — use `internal` access + `@testable import` instead; the precedent is `shouldReportIncomingCallToCallKit`. |
