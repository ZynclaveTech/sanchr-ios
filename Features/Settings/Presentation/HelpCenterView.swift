    ...
    var body: some View {
        ...
        .alert("Emergency Support", isPresented: $showEmergencyInfo) {
            Button("Email emergency@sanchr.io") {
                if let url = URL(string: "mailto:emergency@sanchr.io?subject=Emergency%20Support%20Request") {
                    UIApplication.shared.open(url)
                }
            }
            Button("Cancel", role: .cancel) {}
        } message: {
            Text("If you believe your account has been compromised or you are experiencing harassment, contact our emergency support team immediately.")
        }
    }
}
            ...
            List {
                ...
                Section {
                    Toggle("Include device info", isOn: $includeDeviceInfo)
                        .tint(.sanchrPrimary)
                    Text("Helps us diagnose issues faster. Includes device model, OS version, and app version.")
                        .font(SanchrTypography.captionSmall)
